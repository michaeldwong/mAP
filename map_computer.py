...
def main_map_compute(ground_truth_for_map_cache, detection_results_for_map_cache, args=None):
    ...
    if args is not None and args.ignore is None:
        args.ignore = []

    specific_iou_flagged = False
    if args is not None and args.set_class_iou is not None:
    ...
    if specific_iou_flagged:
        if args is not None:
            ...
            return error('Error, missing arguments. Flag usage:' + error_msg)
        error_msg = \
        ...
        for tmp_class in specific_iou_classes:
            if tmp_class not in gt_classes:
                        return error('Error, unknown class \"' + tmp_class + '\". Flag usage:' + error_msg)
        for num in iou_list:
            if not is_float_between_0_and_1(num):
                return error('Error, IoU must be between 0.0 and 1.0. Flag usage:' + error_msg)
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            if show_animation:
                ...
                if len(ground_truth_img) == 0:
                    return error("Error. Image not found with id: " + file_id)
                elif len(ground_truth_img) > 1:
                    return error("Error. Multiple image with id: " + file_id)
                else: # found image
        ...
        sum_AP += ap
        text = "{0:.2f}".format(ap) + "=" + class_name + " AP " #class_name + " AP = {0:.2f}%".format(ap*100)
        """
        Write to output.txt
        """
        rounded_prec = [ '%.2f' % elem for elem in prec ]
        rounded_rec = [ '%.2f' % elem for elem in rec ]
        if (args is not None and not args.quiet) or (args is None):
            output_str += text
            output_str += "\n"
